 ...
 */
rnd = FW.rnd
function SimulatorRenderer(WIDTH, renderer, birdsPosition) {

  WIDTH = WIDTH || 4;
  var camera = new THREE.Camera();
  camera.position.z = 1;
  this.birdsPosition = birdsPosition;
  ...
  var velocityShader = new THREE.ShaderMaterial( {
    ...
    uniforms: {
      ...
      prey: {type: "v3", value: birdsPosition }
    },
    defines: {
  ...
  function init() {
    var dtPosition = generatePositionTexture(this);
  ...
  function generatePositionTexture(context) {

    var a = new Float32Array( BIRDS * 4 );

    for ( var k = 0; k < BIRDS; k += 4 ) {

      var x = Math.random() * BOUNDS - BOUNDS_HALF;
      var y = Math.random() * BOUNDS - BOUNDS_HALF;
      var z = Math.random() * BOUNDS - BOUNDS_HALF;
      a[ k + 0 ] = x + context.birdsPosition.x;
      a[ k + 1 ] = y + context.birdsPosition.y  
      a[ k + 2 ] = z + context.birdsPosition.z;
      a[ k + 3 ] = 1;
  ...
  function generateVelocityTexture() {
    ...
    var a = new Float32Array( BIRDS * 3 );

    for ( var k = 0; k < BIRDS; k += 3 ) {
